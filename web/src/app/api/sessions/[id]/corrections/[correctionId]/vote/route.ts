...
// POST /api/sessions/[id]/corrections/[correctionId]/vote - Vote on a correction
export async function POST(
  request: NextRequest,
  { params }: { params: Promise<{ id: string; correctionId: string }> }
) {
  const session = await auth();
  const { id: sessionId, correctionId } = await params;
  ...
  try {
    ...
    const correction = await prisma.patternCorrection.findFirst({
      where: {
        id: correctionId,
        sessionId,
        session: {
    ...
    if (!correction) {
      return NextResponse.json(
        { success: false, error: "Correction not found or access denied" },
        { status: 404 }
      );
    }
    ...
    if (value === 0) {
      ...
      if (existingVote) {
        await prisma.$transaction([
          ...
          prisma.patternCorrection.update({
    ...
    return NextResponse.json(
      { success: false, error: "Failed to vote on correction" },
      { status: 500 }
    );
  }
}

// GET /api/sessions/[id]/corrections/[correctionId]/vote - Get user's vote on a correction
